...
public class GenerateCss(KitRepository kits, IWebHostEnvironment env) : Controller
{
    ...
    public async Task<IActionResult> HandleAsync(string kitId, string? v, string? scope)
    {
        ...
        css.AppendLine(GetLocalFile("Models/Elements/_Shared/destyle-reset.css"));

        if (kit.Revision.Settings.UseTypography)
            css.AppendLine(GetLocalFile("Models/Elements/Typography/typography.css"));

        if (kit.Revision.Settings.UseButtons)
            css.AppendLine(GetLocalFile("Models/Elements/Buttons/buttons.css"));

        if (kit.Revision.Settings.UseInputs)
            css.AppendLine(GetLocalFile("Models/Elements/Inputs/inputs.css"));

        if (kit.Revision.Settings.UseAnchors)
            css.AppendLine(GetLocalFile("Models/Elements/Anchors/anchors.css"));

        if (kit.Revision.Settings.UseLists)
            css.AppendLine(GetLocalFile("Models/Elements/Lists/lists.css"));

        if (kit.Revision.Settings.UseShadows)
            css.AppendLine(GetLocalFile("Models/Elements/Effects/shadows.css"));
