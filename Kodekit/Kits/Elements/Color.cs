...
public class Color : ISerializable
{
    ...
    internal Color(string hex) : this(hex, "")
    {
    }

    internal Color(string hex, string defaultHexValue) : this()
    {
    ...
    internal Color(System.Drawing.Color color)
    {
    ...
    internal Dictionary<string, string> Serialize(string? type)
    {
    ...
    internal Dictionary<string, string> Expand(Color color)
    {
        var type = "grey";
        var colors = new Dictionary<string, string>
        {
            { $"{type}-50", Interpolate(color, 0.05f).HexValue }
        };
    ...
    internal string RgbaValue(double opacity)
    {
        var color = ColorTranslator.FromHtml(HexValue);
        return $"rgba({color.R}, {color.G}, {color.B}, {opacity})";
    }

    internal Color Interpolate(Color secondColor, float percentage)
    {
